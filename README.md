...
1. **🔐 新窗口不再反复要管理员权限**：第一次授权后放开 workbench 目录写入权限，之后开新窗口、换背景不再连弹 3 次 UAC。

---

## 🚀 3.5.5 主要更新

1. **🪟 多主窗口独立背景**：两个 Cursor/VS Code 主窗口可以各挂一张图。换图只改当前窗口，点回另一扇窗不会被带跑。关掉再开同一个项目会恢复该项目上次的图；新窗口用原来的默认图。同一窗口里的 Agent Window 仍跟主窗口。

---

## 🚀 3.5.4 主要更新

1. **🔄 自动换图失败静默重试**：超时 / 5xx / 连接抖动会短间隔重下同一张；401/403/404 则换下一张。失败不再弹错，也不会清掉在线文件夹。
...
#### ver 3.5.6 (2026/08/13)

1. 写入失败时改为对 workbench 目录一次性授权（新文件继承写入权限），避免每个新窗口都对单文件弹 3 次管理员确认。

#### ver 3.5.5 (2026/08/13)

1. 多个主窗口可以各自显示不同背景：换图只写当前窗口，不再通过全局设置把所有窗口刷成同一张。关掉再开同一个项目会记住该项目上次的图；新窗口仍用全局默认图。

#### ver 3.5.4 (2026/08/13)

1. 自动换图遇到超时、5xx、连接抖动时会静默重试；单张 404 则换列表里的下一张，不再弹出「更新失败」或清掉在线源。
